...
if __name__ =="__main__":
	...
	fb15k237exp = build_data(name = 'fb15k237',path = tools.cur_path + '/datasets/')


	#SGD hyper-parameters:
	params = Parameters(learning_rate = 0.5, 
						max_iter = 1000, 
						batch_size = int(len(fb15k237exp.train.values) / 100),  #Make 100 batches
						neg_ratio = 10, 
	...
	fb15k237exp.grid_search_on_all_models(all_params, embedding_size_grid = [emb_size], lmbda_grid = [lmbda], nb_runs = 1)


	#Print best averaged metrics:
	fb15k237exp.print_best_MRR_and_hits()



	#Save ComplEx embeddings (last trained model, not best on grid search if multiple embedding sizes and lambdas)
	#e1 = fb15k237exp.models["Complex_Logistic_Model"][0].e1.get_value(borrow=True)
	#e2 = fb15k237exp.models["Complex_Logistic_Model"][0].e2.get_value(borrow=True)
	#r1 = fb15k237exp.models["Complex_Logistic_Model"][0].r1.get_value(borrow=True)
	#r2 = fb15k237exp.models["Complex_Logistic_Model"][0].r2.get_value(borrow=True)
